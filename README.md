# 任务看板

课堂实践项目：支持任务管理、三列看板、拖拽、深色模式和浏览器本地持久化。

## 运行

直接打开 `index.html`，或在项目目录运行 `python -m http.server 8000` 后访问 `http://localhost:8000`。项目无需安装依赖。

## 开发记录

每轮遵循「计划 → 执行 → 浏览器确认 → Git commit」。详细目标与确认记录见 [ITERATIONS.md](ITERATIONS.md)。

## 功能与演示

任务可新增、编辑、删除，标题必填、描述选填。看板有待办、进行中、完成三列，卡片可以拖拽。高、中、低优先级分别显示红、黄、绿。深色模式和任务数据保存在当前浏览器。

课堂演示顺序：展示 Git 提交历史；新建高优先级任务；编辑并拖动到进行中；切换深色模式并刷新；确认内容保留；删除任务。

课件推荐 Vue 3、Vite、Tailwind CSS，本项目采用无依赖 HTML、CSS、JavaScript，直接打开 `index.html` 即可运行。

## 浏览器验证

运行 `python -m http.server 8000`，访问 `http://localhost:8000/tests/browser-smoke.html`，页面显示 PASS 表示核心交互通过。测试会清除当前站点的任务数据，请用独立浏览器配置运行。

## 视觉改版

新增原创 AI 插画 `assets/task-hero.png`，重新安排页面标题、统计与看板的视觉层次。图片由内置 ImageGen 生成，提示词要点：暖白纸张任务清单、珊瑚色铅笔、蓝色缎带、右侧主体、左侧留白、无文字。应用功能和 `localStorage` 键保持不变。

## 卡片操作与视觉细节

任务卡整体按优先级显示淡红、淡黄、淡绿。点击任务卡，会在卡片上显示“编辑”和“删除”；再次点击卡片或点击空白处可收起。键盘可聚焦卡片，按 Enter/空格显示操作，按 Esc 收起。深色模式同步压暗顶部插画。三列标题采用原创装饰图，分别对应待办、进行中、完成。

## 看板本子样式

三列使用整幅原创本子标题图：待办为暖陶色铅笔纸页、进行中为蓝色缎带纸页、完成为鼠尾草绿枝叶纸页。任务卡收紧了上下留白，状态标题加大加粗。图片由内置 ImageGen 生成后压缩为 WebP，保留页面的轻量加载。

## 纸面边缘修正

三张本子标题图换成纸张铺满画布的版本，四边平直，没有撕边或外侧底色。任务区使用配套的细纸纹，深色模式也保持一致的纸面层次。

## 纸纹任务卡与主题色

高、中、低优先级卡片分别使用浅红、浅黄、浅绿的原创纸纹，卡片与标签均改为直角。日间页面底色调成暖米色；夜间适度提亮顶部插画和三列标题图。纸纹使用内置 ImageGen 生成并压缩为 WebP。

## 搜索与卡片排版

统计栏右侧可以按任务标题或描述实时搜索；在窄屏上搜索栏会移到统计项下方。搜索只筛选看板中的卡片，不改变全部任务、进行中和已完成的总数。纸纹任务卡恢复圆角，优先级、标题和描述的文字左边线一致。
